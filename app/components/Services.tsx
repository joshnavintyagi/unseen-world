...
export default function Services() {
  const handleBooking = (href: string) => {
          ...
          {services.map((service) =>
            service.href.startsWith("/") ? (
              <Link
                key={service.id}
                href={service.href}
                className="group overflow-hidden rounded-[18px] border border-[#D4AF37]/25 bg-[#091521] transition-all duration-300 hover:-translate-y-1 hover:border-[#D4AF37] hover:shadow-[0_20px_45px_rgba(212,175,55,0.18)]"
              >
                ...
                <div className="relative px-5 pb-5">
                  ...
                  <div className="mt-5 inline-flex items-center gap-2 font-semibold text-[#D4AF37]">
                    Book Now →
                  </div>
                </div>
              </Link>
              ...
              <article
                key={service.id}
                onClick={() => handleBooking(service.href)}
